...
- **Container issues:** Search logs for `component: "container-manager"`. `docker ps -a --filter label=com.lifemodel.component=motor-cortex` lists all Motor Cortex containers. Stale containers are pruned on restart. Docker is required for agentic runs — if unavailable, they fail with an error.
