...
export class ConversationManager {
  ...
  /**
   * Trim older messages from conversation, keeping recent turns.
   * Uses a CAS guard: if the current message count doesn't match expectedMessageCount,
   * another compaction ran concurrently and this one is stale — returns false.
   *
   * Summaries are stored externally in the memory system (not here).
   *
   * @param userId User ID
   * @param expectedMessageCount Expected message count at time of compaction decision
   * @returns true if trimmed successfully, false if stale (concurrent compaction)
   */
  async compactMessages(userId: string, expectedMessageCount: number): Promise<boolean> {
    const key = this.getKey(userId);
    const stored = await this.loadConversation(key);

    // CAS guard: abort if message count changed (another compaction ran)
    if (stored.messages.length !== expectedMessageCount) {
      this.logger.debug(
        { userId, expected: expectedMessageCount, actual: stored.messages.length },
        'Compaction skipped: message count mismatch (stale)'
      );
      return false;
    }
    ...
    // Update stored data — only trim messages, no summary stored here
  ...
  async getMessagesToCompact(userId: string): Promise<ConversationMessage[]> {
    const { messagesToCompact } = await this.getCompactionSnapshot(userId);
    return messagesToCompact;
  }

  /**
   * Get a compaction snapshot: messages to compact AND total message count from a single read.
   * Used by the compaction flow to ensure the CAS guard covers exactly the messages summarized.
   */
  async getCompactionSnapshot(
    userId: string
  ): Promise<{ messagesToCompact: ConversationMessage[]; totalMessageCount: number }> {
    const key = this.getKey(userId);
    const stored = await this.loadConversation(key);

    const totalMessageCount = stored.messages.length;
    ...
    const messagesToCompact = toCompact
    ...
    return { messagesToCompact, totalMessageCount };
  }
   ...
   * Used by the compaction flow for CAS guard.
   */
